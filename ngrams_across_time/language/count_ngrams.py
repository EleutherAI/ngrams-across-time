...
def parse_args():
    ...
    parser.add_argument('--cont', action="store_true") # Continue
    parser.add_argument('--overwrite', action="store_true")
    parser.add_argument('--debug', action="store_true")
    parser.add_argument('--n', type=int, nargs="+", required=True) # Order of n-grams
...
def main():
    ...
    n_dict = {
        n: (
            data_path / f"{n}-gram-autoregressive-counts-{args.num_shards}-shards{'-debug' if args.debug else ''}-{args.num_samples}.npy",
            data_path / f"{n}-gram-autoregressive-samples-{args.num_shards}-shards{'-debug' if args.debug else ''}-{args.num_samples}.npy"
        )
        for n in args.n
    }

    # Fail early if existing data could be unintentionally overwritten
    for n, (count_file, sample_file) in n_dict.items():
        if args.overwrite or args.debug:
            count_file.unlink(missing_ok=True)
            sample_file.unlink(missing_ok=True)

        if count_file.exists() and not args.cont:
            print(f"Exiting as a counts file exists and cont flag not set: {count_file}")
            exit(1)

        if sample_file.exists() and not args.cont:
            print(f"Exiting as a logprobs file exists and cont flag not set: {sample_file}")
            exit(1)
    
    print(f"Total tokens across all shards: \
          {sum(os.path.getsize(path[0]) // 2 for path in tokengrams_paths[:args.num_shards])}")

    # Load index
    start = time.monotonic()
    index = ShardedInMemoryIndex(tokengrams_paths[:args.num_shards], vocab_size)
    print(f"Loaded index with {len(tokengrams_paths[:args.num_shards])} shards in {time.monotonic() - start}s...")
    ...
    for n, (count_file, sample_file) in n_dict.items():
        count_mmap = np.memmap(
            ...
            dtype=np.int64,
            shape=(args.num_samples, args.seq_len, vocab_size),
        )
        sample_mmap = np.memmap(
            filename=sample_file,
            mode="w+" if not sample_file.exists() else "r+",
            dtype=np.int32,
            shape=(args.num_samples, args.seq_len),
        )

        count_mmap[:, 0] = unigram_counts.numpy()
        unigram_samples = torch.multinomial(unigram_counts.float(), args.num_samples)
        ...
        for column in tqdm(range(1, args.seq_len)):
            ...
            prev = [prev[i] + [samples[i].item()] for i in range(args.num_samples)]
...
def log_probs(n, num_shards, num_samples, seq_len, vocab_size, data_path):
    ...
    for i in range(0, num_samples * seq_len, chunk_size):
        ...
        probs = np.nan_to_num(probs, nan=1e-10) # Uniform prior on unseen n-grams
