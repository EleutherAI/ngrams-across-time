...
def get_args():
    ...
    parser.add_argument("--model_seed", type=int, default=999)
...
def main():
    ...
    if PATCH or LOSS:
        for epoch, state_dict in tqdm(list(zip(checkpoint_epochs, model_checkpoints))):
            ...
            if PATCH and (OVERWRITE or epoch not in checkpoint_eap_data):
                ...
                # normalize scores, then do entropy interpreting them as probs
                # entropy = -sum(p_i * log(p_i)) for p_i in probs
    ...
    fig.add_trace(go.Scatter(x=checkpoint_epochs, y=[len(item) for item in thresholded_scores], mode='lines', name='Number of features above score threshold'))
